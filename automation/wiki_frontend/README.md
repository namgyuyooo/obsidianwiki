...
- 상태와 별개로 사용자 멀티태그를 관리한다. 태그는 쉼표/세미콜론/줄바꿈으로 여러 개 입력할 수 있고, `#` prefix는 저장 시 제거한다.
- 상단 `태그 전체` 필터로 태그별 문서를 볼 수 있으며, 문서 체크박스와 `위키 멀티태그 일괄 관리`에서 선택 문서 태그를 추가하거나 교체할 수 있다.
- 문서 상세 패널의 `사용자 멀티태그` 입력은 해당 문서/프로젝트 운영 태그를 직접 수정한다.
- `GLM_Global_Instructions.md`와 `GLM_Chat_Projects/*` 같은 대화 규칙/보조 메모리 문서는 업무 추진 상태가 아니므로 상태 관리 대상에서 제외한다.
...
- AI UX 원칙은 `가능한 일`, `근거`, `상태/복구`, `메모리`를 사용자가 즉시 볼 수 있게 만드는 것을 기본으로 한다.
- GLM 챗 상단에는 capability strip을 두어 모델이 처리할 수 있는 일과 한계를 명시한다.
- 메시지는 `USER`, `GLM`, `ERROR` 레이블과 상태 chip을 사용해 발화 주체와 실패 지점을 구분한다.
- 복잡한 결과는 채팅 본문에 반복하지 않고, 카드/사이드 패널/결과 뷰어로 분리한다.
- 색상/반경/그림자는 `--ai-*` CSS 토큰으로 관리해 Wiki, Paperclip, Chat 화면이 같은 시각 언어를 공유한다.
...
- HTML로 만든 보고서도 `html/htm` 확장자 기준으로 manifest와 위키화 대상에 포함된다.
- 단, Drive의 `Github`/`GitHub`/`Obsidian_wiki` 계열 폴더는 기본 제외된다. 본 위키가 Drive에 업로드되어 있어 자기 자신을 다시 위키화하는 순환 수집을 막기 위한 안전장치다.
