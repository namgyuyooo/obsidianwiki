...
## 한줄 요약
SCR 압연 설비 이상탐지(SCADA/Monit)와 케이블 외관검사 솔루션을 결합한 데이터 분석+제안형 과제. 현재 핵심 리스크는 데이터 단위/해석 조건 정합성.

## 프로젝트 유형
데이터 분석 + 제안형 / 분석 기반 제안 단계

## 현재 상태
- 불량 샘플 이미지·센서 CSV 이미 공유된 상태
- 데이터 단위 확인(압연 2번=RPM, 12번=Hz)과 가이드 문서 역할 정리가 선행 과제
- 상관관계 분석 기준(r>.9, r>0.8)을 고객용 설명 기준으로 재정리해야 함

## 이번 주 실무 포인트
- 데이터 단위/설정값 정규화
- 외관검사 제안 범위와 이상 탐지 분석 범위 분리
- 고객 질의 대응용 분석 기준 문서화
