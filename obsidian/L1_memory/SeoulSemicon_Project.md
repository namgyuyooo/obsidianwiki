...
## 한줄 요약
Xray Repair 해외 납품 프로젝트. Quotation → NDA → 계약 → PO → Invoice → Acceptance Report까지 완성된 사이클. 4대 설비 정상 가동 확인, 현장 출장 Wrap-up 단계.
...
## 핵심 결정사항
- 2025년: 견적/상업 문서 단계
- 2026년: NDA + 계약 + PO + Invoice + 검수 완전 완료
- 현장 이슈: 네트워크 속도(1000M vs 100M)가 처리 속도에 직접 영향

## 핵심 수치 / 파일
- 처리 속도: 신규 PC(이더넷 1000M) ~15초, 기존 환경(100M) 2분대
- 설비: 4대 복수 설비 가동 안정화 완료
...
- 네트워크 속도·PC 사양이 처리 성능에 직결 — 해외 설치 시 사전 환경 확인 필수
