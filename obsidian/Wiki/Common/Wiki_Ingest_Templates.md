...
## 처리 원칙
- 실제 상충값, 상충 주장, 버전 불일치만 이 문서에 남긴다.
- 확정되면 [[Decisions]]로 승격한다.
- 실행이 필요하면 [[Action_Items]]로 연결한다.
- 리스크가 커지면 [[Risks]]로 승격한다.

## Capture Rule
- 아래 세 조건을 모두 만족할 때만 conflict로 등록한다.
  - 기존 값과 신규 값이 동시에 current truth로 유지되기 어렵다.
  - 어느 쪽을 기준값으로 쓸지 아직 확정되지 않았다.
  - 해소 전까지 KPI, 일정, 범위, 고객 커뮤니케이션 중 하나에 실무 오판 위험이 있다.
- 아래 항목은 conflict로 올리지 않는다.
  - `정합성 확인 필요`
  - `프로젝트 경계/차수/범위 해석`
  - `미팅 때 물어볼 질문`
  - `허브/상태/액션 업데이트 필요`
  - 승인 로그, 위키 승격 로그, 병합 메타데이터
```
